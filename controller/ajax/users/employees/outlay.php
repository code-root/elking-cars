...
if ( isLoginAdmin() !== true && $_SESSION['status'] != 'user' &&  $_SERVER['REQUEST_METHOD'] != 'POST') {  
    ...
    if (!filter_var($_POST['customerID'], FILTER_SANITIZE_STRING)) {
      $message = 'من فضلك إختر الموظف  العميل';
      $code = 1;
      $status = 1;
    } else {
    ...
    if ($status == 0) {
      $sql = "INSERT INTO `outlay` ( `name`,  customerID  , `UserID`, `number_count`, `date`, `status`)
                                 VALUES ( '$name', '$customerID', '$UserIdLogin', '$number_count', '$now', 0);";
      ...
      if ($conn->query($sql) === TRUE) {
          ...
          $code = 200 ;
          echo   UpdateAdmin($_SESSION['name'] , $now , $name  ) ;
